...
class PendataanController_restoran extends GetxController {
  ...
  Future fetch_all() async {
    ...
    const limit = 8;
    final url = Uri.parse(
        '${baseUrlApi}/get_pelaporan_admin/index.php?page=$page&limit=$limit&jenispajak=Restoran');
  ...
  void refreshData() async {
    ...
    const limit = 8;
    final url = Uri.parse(
        '${baseUrlApi}/get_pelaporan_admin/index.php?page=1&limit=$limit&jenispajak=Restoran');
    final response = await http.get(url);
    List newItems = json.decode(response.body);
    final list = newItems
        .map<ModelGetpelaporan>((json) => ModelGetpelaporan.fromJson(json));
