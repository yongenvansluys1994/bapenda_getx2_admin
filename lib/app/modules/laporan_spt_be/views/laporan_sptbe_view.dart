...
class LaporanSPTBEView extends GetView<LaporanSPTBEController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: "Laporan Total SPT & Pajak BE",
        leading: true,
        isLogin: true,
      ),
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Divider(),
            Texts.body1(
                'Laporan Jumlah Pajak yang dilaporkan melalui Bapenda Etam',
                maxLines: 2,
                  ...
                  return SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: [
                        ...
                        DataColumn(label: Text('JML SPT')),
                        DataColumn(label: Text('TOTAL PAJAK')),
                      ],
                      rows: controller.datalist.map((dataItem) {
                        return DataRow(cells: [
                          DataCell(Text("${dataItem.namaPajak}")),
                          DataCell(Text("${dataItem.totalSpt}")),
                          DataCell(Text(
                              "${NumberFormat.currency(locale: 'id', symbol: 'Rp. ', decimalDigits: 0).format(double.parse(dataItem.totalPajak).toInt())}")),
                        ]);
                      }).toList(),
                    ),
                  );
